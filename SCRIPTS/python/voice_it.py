...
# info
novel_title = "My Three Wives are beautiful vampires Novel"
chapter_selector = ""

# config
host =  "https://novelbjn.phieuvu.com"
url_pattern = f"{host}/book/[novel-id]/[chapter-id]"
...
download_config = {"worker_count": 5, "worker_interval": 1}
...
custom_list_path = "list.json" ## chapter_id: url
...
if use_custom_list:
    try:
        with open(custom_list_path, "r") as list_file:
            chapter_list = json.load(list_file)
    except:
        console.print(error_tag, f"Failed to load list: {custom_list_path}")
...
def parse_download_url(novel_id: str, chapter_id: str):
    ...
    if use_custom_list:
        try:
            url = host + chapter_list[chapter_id]
            return url
        except KeyError:
            console.print(error_tag, f"Can't find url for chapter: {chapter_id}")
            sys.exit()
...
## let's make folders
if os.path.exists(folders["temporary"]) and not use_cached:
...
async def download_chapter(chapter: dict):
    if use_cached and os.path.exists(chapter["text_path"]):
...
def extract_html(html: str):
    ...
    if not (title or paragraphs):
        raise Exception("No title or paragraphs found !")
...
async def tts_chapter(chapter: dict):
    ...
    os.rename(chapter["text_path"],
              f"{backup_folder}/{chapter['base_name']}.txt")
    with open(chapter["text_path"], "w") as text_file:
        text_file.write(text)
...
async def tts_text(text: str, output_file: str, advance):
    ...
    try:
        if os.path.exists(output_file) and use_cached:
            return text
...
progress_panel = Panel(progress, title="Wait a minute !")
with Live(progress_panel, console=console, refresh_per_second=2) as live:
