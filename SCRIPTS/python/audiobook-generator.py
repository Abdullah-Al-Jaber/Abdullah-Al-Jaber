...
## pip install rich requests py7zr av bs4 edge-tts eyed3
...
chapter_list_file = "chapter_list.html"

chapter_list_table_selector = "body"
...
make_demo_audio = False

if show_voice_list or make_demo_audio:
...
def run_command(command, show_output=False, show_error=True):
  ...
  if process.returncode == 0:
    ...
    if show_error:
      console.print(error_tag, f"Failed running command: [yellow]{command}[/]")
      console.print(error)
    return False
...
def extract(html_file):
  ...
  if not fresh_mode and os.path.exists(text_file_path):
    if not suppress_logs:
      console.print(success_tag, f"[CACHE] [cyan]{text_file_path}[cyan]")
    _progress.update(_task, advance=1)
    if mini_mode:
  ...
  try:
    ...
    _progress.update(_task, advance=1)
    if not suppress_logs:
      console.print(success_tag, f"[EXTRACT] [magenta]{text_file_path}[/]")
    if mini_mode:
      os.remove(html_file_path)
  except Exception as error:
...
async def generate_audiobook_subtitle_coroutine(chapter_name,
  ...
  async with generate_semaphore:
    ...
    if not dot_exists and files_exists:
      if not suppress_logs:
        console.print(success_tag, f"[CACHE] [cyan]{chapter_name}[cyan]")
      _progress.update(_task, advance=1)
      return 'Horray !'
    ...
    if not suppress_logs:
      console.print(success_tag, f"[GENERATE] [cyan]{chapter_name}[/]")
...
def tag_updater(chapter_name):
  ...
  if process.returncode == 0:
    _progress.update(_task, advance=1)
    if not suppress_logs:
      console.print(success_tag, f"[PLAYLIST] [cyan]{playlist_file_path}[/]")
    if mini_mode:
...
def chapter_urls():
  try:
    ...
    if os.path.exists(chapter_urls_file):
      console.print(success_tag, "[CACHE] Chapter URL file found !")
      with open(chapter_urls_file, "r") as list_file:
        chapter_urls = json.load(list_file)
    else:
      if os.path.exists(chapter_list_file):
        console.print(success_tag, f"Using [cyan]{chapter_list_file}[/]")
        with open(chapter_list_file, "r") as list_file:
          chapter_list_html = json.load(list_file)
      else:
      ...
      soup = soup.select_one(chapter_list_table_selector)
      if not soup:
        console.print(error_tag, "Failed to extract chapter url list !")
        sys.exit()
...
def _trim_urls(range_text):
  ...
  if start > end:
    start, end = end, start
  console.print(info_tag, f"TRIM RANGE: [cyan]{start}-{end}[/]")
  console.print(info_tag, f"TRIM SIZE: {end - start + 1}")
...
async def _download():
  ...
  for chapter_name, url in chapter_urls.items():
    ...
    if html_file in html_files:
      html_file_path = os.path.join(download_directory, html_file)
      if not suppress_logs:
        console.print(success_tag, f"[CACHE]: [cyan]{html_file_path}[/]")
    else:
  ...
  console.print(info_tag, f"File count: {html_file_count}")
...
def playlist():
  is_eyed3_installed = run_command("eyeD3 --version", show_error=False)
  if not is_eyed3_installed:
    console.print(error_tag, "eyeD3 is installing ...")
    if not run_command("pip install eyed3"):
      console.print(error_tag, "Failed to install eyed3 !")
      sys.exit(1)
    else:
      console.print(success_tag, "Installed eyed3 !")
...
try:
  ...
  if auto_mode:
    ...
    remove_empty_directory(extract_directory)
  ...
  console.print(error_tag, "Unknown Exception occured !")
  console.print(error)
  # raise error
